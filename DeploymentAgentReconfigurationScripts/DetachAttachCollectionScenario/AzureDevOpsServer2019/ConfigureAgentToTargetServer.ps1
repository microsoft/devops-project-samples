...
This is a guidance script to re-configure deployment group agents to the target TFS after detach/attach activity. This script is applicable if the source collection is not online. If source TFS collection is online (Side by Side scenario), use ConfigureAgentToTargetServerUsingSourceServer,ps1 script instead.

Inputs:
1. targetTFSUrl : New TFS instance url.
2. targeTFSPatToken : PAT token from target TFS with Deployment group manage scope (at least)
...
5. tags : Tags of the target. Since the source TFS is not available, user need to pass the tags explicitly.
...
Example usage:
.\ConfigureAgentToTargetServer.ps1 -targetTFSUrl <newTfsUrl> -sourceTFSPatToken <PAT-for-old-TFS>
.\ConfigureAgentToTargetServer.ps1 -targetTFSUrl <newTfsUrl> -sourceTFSPatToken <PAT-for-old-TFS> -existingAgentFolder <AgentFoilder (C:\vstsagents\A1)> -tags 'Web,DB'
.\ConfigureAgentToTargetServer.ps1 -targetTFSUrl <newTfsUrl> -sourceTFSPatToken <PAT-for-old-TFS> -existingAgentFolder <AgentFoilder (C:\vstsagents\A1)> -action 'apply'
#>
...
$encodedPat = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(":$targetTFSPatToken"))
...
if ($action -ne "apply"){
    Write-Verbose -Verbose "If action is set to apply, this script will configure an agent with name $agentName and tags $tags in $newAgentPath path to the deployment group $deploymentGroupName in the TFS $targetTFSUrl with same properties as the existing agent $agentName in the source TFS $sourceTFSUrl.";
    return 0;
}

Write-Verbose -Verbose "Start execution : It will configure an agent with name $agentName and tags $tags in $newAgentPath path to the deployment group $deploymentGroupName in the TFS $targetTFSUrl with same properties as the existing agent $agentName in the source TFS $sourceTFSUrl.";
